...
impl <O> GenSimpleFn<O> {
    pub fn new(fun: fn(Point2<i32>) -> O) -> Rc<GenSimpleFn<O>> {
        Rc::new(GenSimpleFn {
            fun: fun
        })
    }
...
impl <O> GenLayer<O> for GenSimpleFn<O> {
    fn gen(&self, _seed: i64, pos: Point2<i32>, size: Vector2<u32>) -> GenUnit2<O> {
...
impl <I: Copy, O> GenLayer<O> for GenSimpleFnTransformer<I, O> {
    fn gen(&self, seed: i64, pos: Point2<i32>, size: Vector2<u32>) -> GenUnit2<O> {
